...
interface CertificationItemProps {
  item: {
    ...
    bgOffsetX?: string;
    bgPosition?: string;
    bgLeftFadeEnd?: string;
    bgRightFadeStart?: string;
    verifyUrl: string;
  };
}
    ...
    <div className="relative w-full overflow-hidden rounded-lg min-h-[150px] md:min-h-[170px]">
      <div className="absolute inset-0">
        <div
          ...
          style={{
            WebkitMaskImage:
              `linear-gradient(to right, transparent 0%, black ${item.bgLeftFadeEnd ?? '14%'}, black ${item.bgRightFadeStart ?? '86%'}, transparent 100%)`,
            maskImage:
              `linear-gradient(to right, transparent 0%, black ${item.bgLeftFadeEnd ?? '14%'}, black ${item.bgRightFadeStart ?? '86%'}, transparent 100%)`,
          }}
        >
          <Image
            ...
            style={{
              ...
              transform: item.bgFlipHorizontal
                ? `translateX(${item.bgOffsetX ?? '0'}) scaleX(-1) scale(${item.bgScale ?? 1.05})`
                : `translateX(${item.bgOffsetX ?? '0'}) scale(${item.bgScale ?? 1.05})`,
            }}
            sizes="100vw"
          />
        </div>
        ...
        <div className="flex items-center gap-4 w-full h-full">
          <div className="w-24 md:w-28 h-20 md:h-24 shrink-0 overflow-hidden rounded-md bg-black/35 p-2">
            <Image
              ...
              style={{
                transform: `translateY(${item.logoOffsetY ?? '0'}) scale(${item.logoScale ?? 1})`,
              }}
              sizes="(max-width: 768px) 100vw, 50vw"
            />
          </div>
          <div className="flex-1 text-left flex flex-col">
            <p className="text-cyber-300 mt-1 font-semibold">{item.fullName}</p>
            <div className="mt-2">
